...
|--------|-------------|----------|
| `DOTNET_SIGNING_CERTIFICATE_BASE64` | Base64 encoded PFX certificate | When `sign-output: true` |
| `DOTNET_SIGNING_CERTIFICATE_PASSWORD` | Certificate password | When using a certificate |
| `DOTNET_NUGET_RESTORE_CREDENTIALS` | PAT (`read:packages`) for restoring from the private feed named by `nuget-source-name` - see [Private NuGet Feeds](#private-nuget-feeds) | When `nuget-source-name` is set |
| `DOTNET_NUGET_API_KEY` | **Deprecated and unused.** It never authenticated anything; it only triggered a step that overwrote the repository `nuget.config`. Still accepted so existing callers do not break, removed in the next major. | Never |
