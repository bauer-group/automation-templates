...
try:
    ...
    print("[error] Paket 'requests' fehlt. Installation: pip install -r requirements.txt", file=sys.stderr)
    raise
...
def apply_protection(cfg: Config, repo: str) -> None:
    ...
    if cfg.dry_run:
        ...
        return
    resp = SESSION.put(url, json=build_payload(cfg), headers=_auth_headers(cfg.token), timeout=TIMEOUT)
    if resp.status_code not in (200, 201):
        print(f"[warn] Schutz fehlgeschlagen für {repo}@{cfg.branch}: {resp.status_code} {resp.text[:160]}")
        return
    ...
    if cfg.require_signed_commits:
        ...
        if sig_resp.status_code in (201, 204):
            ...
            print(f"[warn] Signaturen nicht aktiviert für {repo}: {sig_resp.status_code} {sig_resp.text[:100]}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if cfg.org:
        try:
            target_repos: Iterable[str] = list_org_repos(cfg.org, cfg.token)
